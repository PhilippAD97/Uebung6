...
public class AusleihWerkzeug {
	...
	public AusleihWerkzeug(MedienbestandService medienbestand, KundenstammService kundenstamm,
		...
		// UI erzeugen
		_ausleiheUI = new AusleiheUI(_kundenAuflisterWerkzeug.getUIPanel(), _medienAuflisterWerkzeug.getUIPanel(),
				_kundenDetailAnzeigerWerkzeug.getUIPanel(), _medienDetailAnzeigerWerkzeug.getUIPanel());
	...
	private boolean istAusleihenMoeglich() {
		...
		boolean ausleiheMoeglich = (kunde != null) && !medien.isEmpty()
				&& _verleihService.sindAlleNichtVerliehen(medien) && _verleihService.istKundeimmerErsterVormerker(medien, kunde);
	...
	private void leiheAusgewaehlteMedienAus() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, exception.getMessage(), "Fehlermeldung", JOptionPane.ERROR_MESSAGE);
		}
	}
