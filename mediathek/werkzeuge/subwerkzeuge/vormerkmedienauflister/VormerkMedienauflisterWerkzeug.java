	...
	private void setzeAnzuzeigendeMedien() {
		...
		for (Medium medium : medienListe) {
			...
			if (_verleihService.istVerliehen(medium)) {
				...
				{
				vormerker1 = _verleihService.getVormerkkarte(medium).getVormerker(1);
				vormerker2 = _verleihService.getVormerkkarte(medium).getVormerker(2);
				vormerker3 = _verleihService.getVormerkkarte(medium).getVormerker(3);
				}
			}
				medienFormatierer
						.add(new VormerkMedienFormatierer(medium, entleiher, vormerker1, vormerker2, vormerker3));
